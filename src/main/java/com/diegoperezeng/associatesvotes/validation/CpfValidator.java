...
public class CpfValidator implements ConstraintValidator<ValidCpf, String> {
	private static final Pattern CPF_PATTERN_POINTS = Pattern.compile("^\\d{3}\\.\\d{3}\\.\\d{3}\\-\\d{2}$");
	private static final Pattern CPF_PATTERN_ELEVEN = Pattern.compile("^\\d{11}$");
	...
	public boolean isValid(String value, ConstraintValidatorContext context) {
		...
		if (!CPF_PATTERN_POINTS.matcher(value).matches() && !CPF_PATTERN_ELEVEN.matcher(value).matches()) {
			return false;
		}
